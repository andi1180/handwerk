...
### Medien-Purge — [lib/media/purge-order-media.ts](lib/media/purge-order-media.ts) (geteilt, server-only)

Speicherplatz freigeben, **ohne die Analytics-Historie anzutasten**. `purgeOrderMedia(client, { orderId, businessId })` ist die **einzige** Stelle mit dieser Lösch-Logik — genutzt vom Portal-Route-Handler und (später) vom Aufräum-Cron, damit beide Wege garantiert identisch löschen.

Was passiert: (1) alle `order_media`-**Zeilen** des Auftrags löschen (Tabelle bleibt); (2) alle Storage-Dateien unter `{business_id}/{order_id}/` im Bucket `order-media` — die von `order_media` referenzierten Dateien, `reel.mp4`, `business-reel.mp4`, `reel-test.mp4` und alle `*.frame-0/1/2.jpg` ([video-frames.ts](lib/media/video-frames.ts)); (3) `booklets.reel_status` bzw. `.business_reel_status` → `'purged'`, **nur aus `'ready'` heraus** (dieselbe Regel wie der Nachlauf in Migration 0014: `'purged'` heißt „es gab ein fertiges Reel, das gelöscht wurde" — nie gerendert bleibt `'pending'`, gescheitert bleibt `'failed'`).

**Nicht angefasst:** die `orders`- und `booklets`-Zeilen selbst sowie `booklet_events`, `billing_events`, `analytics_events`. Genau dafür existiert die Funktion — statt den Auftrag zu löschen.

Die Löschmenge wird aus dem **Listing** des Auftragsordners bestimmt (nicht nur aus den DB-Zeilen), damit auch Dateien ohne Zeile erfasst werden: Reel-Ausgaben (per Konvention ohne Zeile) und verwaiste Frames. Reihenfolge Storage → Zeilen → Status: scheitert der Storage-Teil, bleiben die Zeilen als Pfad-Referenz für einen Retry erhalten. Die Funktion **wirft nicht**, sondern liefert `PurgeOrderMediaResult` (`ok`, gelöschte Zeilen/Dateien, Bytes/MB, aktualisierte Status, `errors[]`) — idempotent, mehrfach aufrufbar.

**ISOLATION (§14.2):** `businessId` kommt vom Aufrufer (Route: aus der Session) und wird **jeder** Query als Filter mitgegeben, zusätzlich zur RLS. Nie aus Body oder Daten abgeleitet. Der Client-Parameter ist ein generischer `SupabaseClient` ⇒ Route nutzt den AUTHENTICATED Client (RLS), der Cron später `service_role`.

### Route Handler — Purge ([…/media/purge/route.ts](app/api/portal/orders/[id]/media/purge/route.ts), `POST`)

Kein Body. AUTHENTICATED Server-Client, `getCurrentBusiness` → 401/403, Order über RLS → 404, dann **nur** der Aufruf von `purgeOrderMedia` — die Route enthält selbst keine Lösch-Logik. Antwort = das Result-Objekt; `ok: false` ⇒ 500 `purge_failed` + `console.error` mit `order_id`/`business_id`/`errors`. Statisches Segment `media/purge` liegt konfliktfrei neben `media/[mediaId]` (wie `media/reorder`).

**UI:** [components/purge-media-button.tsx](components/purge-media-button.tsx) — Icon-Button auf **archivierten** Auftragskacheln, sichtbar nur im Archiv-Scope und nur solange nicht **beide** Reel-Status `'purged'` sind. Bestätigung über ein div-Overlay im bestehenden Popup-Muster (`biz-reel-backdrop`/`biz-reel-popup`), **kein** natives `confirm()`; das Overlay nennt Kunde + roapp-Nummer. Propagation-stoppend (die Kachel ist ein `<Link>`).

**`'purged'` in der Anzeige** (gilt rückwirkend auch für die per 0014 bereinigten Aufträge): `BusinessReelStatus` und `ReelStatus` kennen den Wert; `BusinessReelButton` zeigt einen nicht-klickbaren „Medien gelöscht"-Pill (Endzustand — kein Render/Teilen/Retry), das Listen-Badge einen neutralen „Medien gelöscht"-Zustand, die Detailseite in der Aktionszone einen Hinweis statt des irreführenden „Läuft im Hintergrund". Auf der Kachel ersetzt der Hinweis den Reel-Button **auch dann, wenn nur einer der beiden Status `'purged'` ist** — sonst böte die Karte einen Render an, der mangels Quellmedien zwangsläufig scheitern würde. Ein „Reel ansehen"-Link entsteht nicht, weil `reelUrl` nur bei `reel_status='ready'` signiert wird.
...
`extractAndUploadFrames(videoFile, videoStoragePath)` läuft in `runUpload` **nach** dem erfolgreichen Video-Upload + `router.refresh()` (Video erscheint sofort), **fire-and-forget**: extrahiert, lädt jeden gültigen Frame per **`uploadWithRetry`** (wiederverwendet) direkt in `order-media` unter `videoFramePath(...)` (`image/jpeg`, `upsert`). **ISOLATION:** Direkt-Upload unter dem RLS-skopierten `{business_id}/{order_id}/`-Präfix (aus dem Video-Pfad), **keine** business_id aus dem Client, **kein** Metadaten-POST (Frames sind keine `order_media`-Zeilen). Schlägt alles fehl ⇒ Video bleibt unverändert (heutiges Verhalten), nur ohne Frames — **kein Breaking**.

> **Update (Video-Tile-Regression-Fix):** Der **zweite** `router.refresh()` am Ende von `extractAndUploadFrames` re-signierte **alle** Signed-URLs und tauschte `<video src>` jeder Kachel aus → auf iOS malte der Video-Layer über die z-index-Controls + lud die Poster neu (sichtbare Regression). Fix: nach dem Frame-Upload ruft `extractAndUploadFrames` jetzt **`createSignedUrls`** (batch, AUTHENTICATED, RLS) für die eben hochgeladenen Frame-Pfade auf und übergibt die URLs via `onFramesUploaded?.(storagePath, urls)` — **kein zweiter `router.refresh()`** mehr. Neuer **Client-Wrapper `MediaSection`** ([media-section.tsx](app/portal/orders/[id]/media-section.tsx), `"use client"`) hält `extraFrames: Map<mediaId, string[]>`, nimmt den Callback von `<Capture>` entgegen (findet die `media.id` per stable `itemsRef` über `storage_path`), merged die URLs in die `initialItems` für `<MediaList>` und entfernt die supplementalen Einträge wieder, sobald der Server nach dem ersten `router.refresh()` server-seitige `frameUrls` liefert (Duplikat-Schutz); `page.tsx` rendert `<MediaSection>` statt `<Capture>` + `<MediaList>` getrennt. **Härtung:** `.media-tile-delete` + `.media-tile-indicator` bekommen `transform: translateZ(0)` ([globals.css](app/globals.css)) → eigene GPU-Compositing-Layer → rendern zuverlässig über dem `<video>`-Layer. **Viewer-Display:** `MediaViewer` bekommt `overflowY: auto` am Outer-Container, der Medien-Bereich `flexShrink: 0` + `maxHeight: 55dvh` → Kategorie, Caption-Editor und Frame-Thumbnails sind auf kurzen Handy-Schirmen ohne Scrollen erreichbar.

> **Update (Play-Overlay sichtbar):** Das dekorative Video-Play-Overlay (`.media-tile-play`, TileVisual) hatte **weder** `z-index` **noch** die `transform: translateZ(0)`-Compositing-Hilfe und war dadurch auf iOS **unsichtbar** — derselbe native-`<video>`-Renderer, der die Ecken-Buttons verdeckte (s. o.), malte über den Play-Glyph. Fix (NUR [globals.css](app/globals.css), Markup unverändert): `.media-tile-play` bekommt `z-index: 2` (unter den tappbaren Ecken-Buttons bei z-index 3, aber über dem `<video>`), der sichtbare Kreis `.media-tile-play span` bekommt `transform: translateZ(0)` (eigene GPU-Compositing-Layer über dem Video-Layer) und wird klarer sichtbar (48×48, `rgba(0,0,0,0.55)`, weißer Glyph, `pointer-events: none`). Gilt für Prozess-Kacheln **und** before/after-Slots (geteilter `TileVisual`), nur Video.
...
## Medien-Upload-Refactor: Buttons raus, Upload via Slots + Prozess-„+"-Tile, Slots bei null Medien

**Reines Frontend** — keine Migration, kein Backend, keine Server-Route. Die Server-Guards (`category_taken`/`limit_reached` in [media/route.ts](app/api/portal/orders/[id]/media/route.ts)) bleiben der harte Riegel; alle UI-Sperren hier sind reine UX. Betroffen: [capture.tsx](app/portal/orders/[id]/capture.tsx), [media-section.tsx](app/portal/orders/[id]/media-section.tsx), [media-list.tsx](app/portal/orders/[id]/media-list.tsx), [de.ts](lib/i18n/de.ts), [globals.css](app/globals.css). Kein `<form>`, kein `any`.

**Idee:** Die zwei „Foto/Video hochladen"-Buttons im Capture entfallen. Der Upload wird jetzt **dort angestoßen, wo das Medium hingehört** — die leeren **Vorher/Nachher-Slots** starten den Foto-Upload ihrer Kategorie (existierte schon, 0010), und ein neues, statisches **Prozess-„+"-Tile** als letztes Kind des Prozess-Rasters startet Foto **oder** Video (Kategorie `process`). Capture hält nur noch die versteckten Inputs, den Entwurf-Dialog und die Upload-Queue; **die gesamte Pipeline (Kompression / Längen-Check / Hintergrund-Upload + Retry/Queue / Metadaten-POST / Frame-Extraktion) wird unverändert wiederverwendet** — keine Duplikation.

**capture.tsx:** Die `.capture-actions`/`.capture-row` mit den zwei Hochladen-Buttons + der standalone Limit-Hinweis-Block (dessen Info jetzt auf Slots/Tiles liegt) + die `UploadIcon`-Deko sind raus. Die versteckten Inputs (`photoUploadInputRef`/`videoUploadInputRef`) + Handler (`handlePhotoFile`/`handleVideoFile`, `replaceDraft`, Entwurf-Dialog) **bleiben** — sie werden jetzt von den Slots/Tiles getriggert. `openPhotoForCategory(cat)` (0010) bleibt der Foto-Opener (inkl. belegter-Slot-/Foto-Limit-Guard + Notice); **neu** `openProcessVideo()` als Video-Opener (klickt `videoUploadInputRef` → `handleVideoFile` erzwingt Kategorie `process`, derselbe `videoLimitReached`-Guard + Notice wie der frühere Button). Beide Opener werden per Ref nach oben gereicht (`openPhotoControlRef` aus 0010 + **neu** `openProcessVideoControlRef`). **Neu** Callback-Prop `onLimitsChange?(photoReached, videoReached)`, in einem `useEffect` bei Änderung von `photoLimitReached`/`videoLimitReached` aufgerufen — spiegelt den Limit-Zustand reaktiv nach oben.

**media-section.tsx (Brücke):** hält `photoLimitReached`/`videoLimitReached` als State (aus `onLimitsChange`) und reicht sie an `<MediaList>`; registriert den Prozess-Video-Opener (`openProcessVideoControlRef`) und reicht `onRequestProcessVideo()` an `<MediaList>`; `requestPhotoForSlot` auf `MediaCategory` verbreitert (Prozess-Foto = `onRequestPhotoForSlot('process')`). Ohne gemountetes Capture (readOnly) bleiben die Limits `false` — die readOnly-MediaList blendet die Bedienelemente ohnehin aus.

**media-list.tsx:** **(1) Early-Return** bei `items.length === 0` nur noch **im readOnly-Modus** (`items.length === 0 && readOnly`) → der „Noch keine Medien"-Card; im **Editier-Modus** rendert immer das normale Layout (Vorher/Nachher-Slots + Prozess-Bereich mit „+"-Tile), auch bei null Medien. **(2) Vorher/Nachher-Slots:** leerer Slot bleibt tappbares „+"; **neu** bei `photoLimitReached` (Prop `addDisabled`) wird der leere Slot gedämpft (`opacity .45`, `pointer-events:none`) + nicht tappbar (sie sind Fotos). **(3) Prozess-Bereich:** der reine Text `media-process-empty` entfällt; stattdessen ein statisches **`<ProcessAddTile>`** als **letztes Kind** von `.media-grid` (nach dem `.map`, **NICHT** in `SortableContext` registriert ⇒ nicht ziehbar, kein Droptarget; `SortableContext`-ids bleiben `processItems.map(m=>m.id)`, `handleDragEnd` unverändert). Tap aufs Tile öffnet eine kleine Foto/Video-Auswahl **direkt im Tile** (kein Overflow-Clipping): „Foto" (deaktiviert bei `photoLimitReached` → `onRequestPhotoForSlot('process')`), „Video" (deaktiviert bei `videoLimitReached` → `onRequestProcessVideo()`); ein `position:fixed`-Backdrop schließt bei Klick daneben (Menü `z-index:31` über Backdrop `z-index:30` im Stacking-Kontext des geöffneten Tiles). Das „+"-Tile wird **nicht** gerendert, wenn Foto- **und** Video-Limit erreicht sind; im readOnly-Modus gibt es weder „+"-Tile noch Auswahl. Das Prozess-Raster ist nun via `!readOnly || processItems.length > 0` gegated (Editier-Modus immer, readOnly nur mit Prozess-Bildern).

**i18n** ([de.ts](lib/i18n/de.ts)): neue `assembler.addProcess` („Medium hinzufügen", Tile-Label/aria), `assembler.chooseFoto` („Foto"), `assembler.chooseVideo` („Video"); Limit-/Belegt-Hinweise nutzen die vorhandenen Keys (`capture.limitReached`/`capture.categoryTakenNotice` etc.). `assembler.processEmpty` entfernt (nicht mehr referenziert). **CSS** ([globals.css](app/globals.css)): `.media-process-add` (thumbnail-groß, gestrichelt, „+", Gold-Hover; analog `.media-ba-empty--add`) + `.media-process-add-menu`/`-option`/`-backdrop`/`-label`; `.media-process-empty` entfernt. Die nur noch von der entfernten Button-Zeile genutzten `.capture-actions`/`.capture-row` bleiben als toter, harmloser CSS-Rest stehen (`.capture-btn` ist mit generate-controls geteilt und bleibt unverändert). `pnpm typecheck` + `pnpm build` grün.
...
> **Schritt 2a — `frames.ts`: `logoPosition` + `bakeBusinessPhotoFrame` (additiv, kein Verhalten geändert):** Vorbereitung für `renderBusinessReel` (Schritt 2b). Zwei additive Erweiterungen in [lib/reel/frames.ts](lib/reel/frames.ts), keine anderen Dateien, keine Migration. **(1) `buildCaptionOverlay`** bekommt optionalen Parameter `logoPosition?: 'topleft' | 'topright'` (Default `'topleft'` → alle bestehenden Aufrufer in `bakePhotoFrame`/`normalizeClip` unverändert, Kunden-Reel byte-identisch). Bei `'topright'`: kleinere Logo-Box 280×80, Position `overlay=W-w-44:44` (rechts oben, 44 px Rand) — kein Konflikt mit dem VORHER/NACHHER-Label links oben. **(2) Neue exportierte Funktion `bakeBusinessPhotoFrame`** (analog `bakePhotoFrame`): cover-crop 1080×1920, Caption-Overlay + Logo **rechts oben** (`logoPosition:'topright'`), plus optionales `label?: 'VORHER' | 'NACHHER'` (drawtext mit `box=1`, `x=80 y=80`, Fontsize 76, Anthrazit-Box bei VORHER / `primaryColor`-Box bei NACHHER, `boxborderw=24`, `expansion=none`, 6.0.1-sicher) — Label sitzt NACH dem Caption-Overlay, liegt also visuell ganz oben. Ausgabe: PNG via `-frames:v 1` (identisch zu `bakePhotoFrame`, damit `encodeStillSegment` + `concatSegments` direkt wiederverwendet werden können). `pnpm typecheck` + `pnpm build` grün.
...
> **Schritt 2 — Archiv-Scope + einzelnes Archivieren/Entarchivieren:** Archiv-Ansicht über `?archived=1` (dieselbe `orders/page.tsx`, kein zweiter Listen-Code); Hauptliste filtert `archived_at IS NULL`, Archiv-Ansicht filtert `archived_at IS NOT NULL`. Archivierbar (Eligibility) = `status ∈ {sent, viewed, shared}` ODER `picked_up_at IS NOT NULL`; Entarchivieren IMMER erlaubt. Geteilter Helfer `lib/orders/archive.ts` (`isArchivable`) — EINE Quelle für UI-Gating (Kachel-Icon nur bei archivierbaren Aufträgen) und Server-Guard (409 `not_archivable`). Route `POST /api/portal/orders/[id]/archive` (AUTHENTICATED, `{ archive: boolean }`). Icon-Button `<ArchiveToggle>` (Client-Insel, `stopPropagation`). `buildOrdersUrl` + `OrdersPagination` führen `?archived=1` durch. KEIN Bulk.

> **Schritt 3a — Mehrfach-Auswahl + Bulk-Archivieren der gewählten IDs:** Client-Auswahl-Schicht über die server-gerenderte Liste via **React Context** (`components/order-bulk-archive.tsx`): `OrderBulkSelectProvider` wrappt die Liste (server-gerenderte Kacheln als Children), der Header-Toggle `SelectModeToggle` („Auswählen" ⇄ „Abbrechen"), die Kachel-Kontrolle `OrderRowControls` und die fixierte `BulkArchiveBar` lesen den Context — die Liste bleibt Server-Component, `<Link>`/Einzel-Archiv-Icon brechen nicht. **Checkbox NUR auf archivierbaren Kacheln** (`isArchivable`): `OrderRowControls` zeigt im Select-Mode eine Auswahl-Checkbox (propagation-stoppender `<button>` im `<Link>` — exakt das `ArchiveToggle`-Muster), sonst das Einzel-Archiv-Icon (Hauptliste) bzw. Entarchivieren-Icon (Archiv-Scope). **Auswahl-UI nur im Hauptlisten-Scope** (kein `SelectModeToggle` im Archiv ⇒ `selectMode` bleibt false). Toolbar ab ≥1 Auswahl: „{n} ausgewählt" + „Archivieren" (inline-Bestätigung „{n} Aufträge archivieren?" Ja/Abbrechen, KEINE stille Aktion) + „Auswahl aufheben"; Auswahl/Modus werden bei Listen-Wechsel (Pagination/Refresh) zurückgesetzt. **Bulk-Endpoint** `POST /api/portal/orders/archive-bulk` (NEU, AUTHENTICATED, kein `service_role`; Body `{ ids: string[], archive: true }`; leer/`>500` ⇒ 400): **EIN UPDATE** `orders SET archived_at=now() WHERE id IN (ids) AND business_id=<Session> AND archived_at IS NULL AND (status IN (sent,viewed,shared) OR picked_up_at IS NOT NULL)` via `.in().eq().is().or(...)` + `.select("id")` ⇒ `{ archived: <count> }`; Eligibility-WHERE spiegelt `isArchivable` über die geteilte `ARCHIVABLE_STATUSES`-Liste (`lib/orders/archive.ts`, kein Drift); nicht-archivierbare/fremde IDs werden still übersprungen, `business_id` NIE aus dem Body. KEIN „Alle auswählen", KEIN Per-Filter-Bulk, KEINE Auto-Archivierung, KEIN Bulk-Unarchive (3b). i18n `orders.select`/`cancel`/`selected`/`archiveSelected`/`clearSelection`/`confirmArchive`/`yes`.

> **UI — Header-Controls Archiv + Auswählen in Dropdown „Archiv ▾" zusammengefasst (vorbereitet für 3b „Alle erledigten archivieren"):** reine UI, kein Backend, keine neue Funktion, keine Migration. Die zwei separaten Hauptlisten-Header-Controls (Archiv-Link → `?archived=1` und „Auswählen"-Toggle) wandern in **ein** Dropdown „Archiv ▾" (`OrdersRefreshButton` bleibt daneben). Kein bestehendes Dropdown-Muster (nur native `<select>` im Status-Filter) ⇒ neue minimale, abhängigkeitsfreie Client-Komponente `components/dropdown-menu.tsx`: `DropdownMenu` (Trigger + Chevron, Klick-außerhalb via `pointerdown` + Escape schließen, Listener nur solange offen) + `DropdownItem` (`href` ⇒ navigierender `<Link>`, sonst `<button onSelect>`; beide schließen das Menü via interner Context-`close()`). `SelectModeToggle` entfernt → `OrdersArchiveMenu` (`order-bulk-archive.tsx`, gleicher Bulk-Select-Context): Trigger „Archiv" + zwei Einträge „Archiv" (`buildOrdersUrl({archived:true})`) und „Auswählen" (`enterSelectMode`); strukturell für den dritten 3b-Eintrag vorbereitet (Platz im Menü, noch nicht eingebaut). Auswahl-Modus exakt wie heute, ohne Doppelung: im Select-Mode wird derselbe Header-Slot zum direkten „Abbrechen"-Button (`exitSelectMode`, kein Menü), die Toolbar trägt weiter nur „Auswahl aufheben". Archiv-Scope (`?archived=1`) unverändert (← Hauptliste, kein Dropdown/„Auswählen"). Keine Logik-Änderung — nur die zwei Trigger wandern ins Dropdown. i18n `orders.archiveMenu` neu (Trigger), Einträge nutzen bestehende `orders.archiveView`/`select`/`cancel`. CSS `.dropdown`/`.dropdown-chevron`/`.dropdown-menu`/`.dropdown-item`.

> **Schritt 3b-1 — geteilter Filter-Query-Bau (Refactor, KEIN Verhalten):** Der inline-Filter-Bau der Auftragsliste (`app/portal/orders/page.tsx`) ist in `buildFilteredOrdersQuery(supabase, opts)` ([lib/orders/orders-query.ts](lib/orders/orders-query.ts), NEU, server-data; **nicht** in `filters.ts`, das von Client-Komponenten importiert wird) extrahiert — damit der spätere by-filter-Bulk dieselbe Filter-Verzweigung anwendet. Signatur `opts = { businessId, status, quick, archived, draftWithMediaIds, selectCols, count? }`; die Funktion baut `.from("orders").select(selectCols [+ count], …).eq("business_id", …)` + Archiv-Scope (`archived` ⇒ `not archived_at is null` / sonst `is null`) + Filter-Verzweigung (quick `flagged`, status `new`/`in_progress`/`generated|sent|viewed|shared`) und liefert den Builder **OHNE** `.order()`/`.range()` (die hängt der Aufrufer an). **`in_progress`** hängt `order_media!inner(id)` an `selectCols` (greift auch für die Bulk-Variante mit `select("id")`); **`new`** nutzt das übergebene `draftWithMediaIds`-Set fürs `NOT IN`. `draftWithMedia` wird wie bisher in `page.tsx` vorab geladen (speist auch das `hasMedia`-Badge) und durchgereicht. Die `count`-Option ist optional (Liste: `"exact"`; Bulk später: weglassen). Client-Typ via `Awaited<ReturnType<typeof createClient>>` (kein `any`, `import type`); `isOrderStatus`-Guard aus `order-status-badge.tsx` für die letzte Status-Verzweigung. Die Liste konsumiert den Builder bit-identisch (gleiche Aufträge/Pagination/count pro Filter). Vorbereitung by-filter-Bulk.

> **D — Composite-Status-Filter (`creating`/`ready`/`failed`), SELECT-only:** Der Status-Filter wird an das zusammengesetzte Listen-Badge (Schritt C) angeglichen — die eine `generated`-Dropdown-Option ist durch **drei** abgeleitete Render-Filter ersetzt, jeder = `status='generated'` UND eine `reel_status`-Bedingung. **(1)** [lib/orders/orders-query.ts](lib/orders/orders-query.ts): in `buildFilteredOrdersQuery` drei neue Zweige — `creating` ⇒ `.eq("status","generated").in("booklets.reel_status",["pending","rendering"])`, `ready` ⇒ `.eq("status","generated").eq("booklets.reel_status","ready")`, `failed` ⇒ `.eq("status","generated").eq("booklets.reel_status","failed")`. Der Embed `, booklets!inner(reel_status)` wird an `selectCols` **nur** bei diesen drei angehängt (analog `order_media!inner` bei `in_progress`); `booklets` ist 1:1 zu `orders` (`order_id` UNIQUE, 0001) ⇒ der Inner-Join bläht `count` NICHT auf, Pagination bleibt korrekt. Die übrigen Zweige (`new`/`in_progress`/`flagged`/`sent`/`viewed`/`shared`) unverändert; die drei decken den gesamten `generated`-Raum ab (`reel_status` NOT NULL). **(2)** [lib/orders/filters.ts](lib/orders/filters.ts): `STATUS_FILTERS` = `["new","in_progress","creating","ready","failed","sent","viewed","shared"]` (`generated` raus); `isStatusFilter` leitet sich daraus ab. `quick='flagged'` referenziert `generated` weiter als Status-WERT (nicht als Filter-Option) ⇒ unverändert. **(3)** [components/order-status-filter.tsx](components/order-status-filter.tsx): die „Generiert"-Option durch „Wird erstellt …" / „Fertig" / „Fehler" ersetzt (`FILTER_LABEL_KEY` → `orderStatus.creating`/`ready`/`failed`, existieren aus C). **(4)** [app/portal/orders/page.tsx](app/portal/orders/page.tsx) **unverändert** — kein Spezial-Handling des `generated`-Filterwerts; `selectCols`-Basis am Aufrufpunkt bleibt (den Embed hängt die Funktion an), die `reelByOrder`-Zweitquery + der Badge-Daten-Pfad sind unberührt (das Badge holt `reel_status` weiter über `reelByOrder`; der Embed dient NUR dem Filtern). Alte `?status=generated`-Links fallen auf „Alle" (kein gültiger Filter mehr). **(5)** `orderStatus.generated` aus [lib/i18n/de.ts](lib/i18n/de.ts) entfernt (nach dem Dropdown-Umbau nirgends mehr referenziert — das Badge nutzt `creating`/`ready`/`failed`, der `flagged`-Quick den Status-Wert ohne Label, und der Badge-`t(\`orderStatus.${status}\`)` narrowt `generated` heraus). SELECT-only — KEIN Bulk, KEIN UPDATE. `pnpm typecheck` + `pnpm build` grün.

> **Schritt 3b-2a — „Alle erledigten archivieren" (Auto-Archiv, ein direktes UPDATE):** ein Knopf, der ALLE archivierbaren aktiven Aufträge des Betriebs auf einmal archiviert, nach Bestätigung mit echter Stückzahl. **KEIN Per-Filter, KEIN `buildFilteredOrdersQuery`** (das ist 3b-2b). Keine Migration. **(1)** [archive-bulk/route.ts](app/api/portal/orders/archive-bulk/route.ts): **NEUER `GET ?scope=all-done`** (Auth wie POST, 401/403; `business_id` aus Session, nie Body) zählt `.from("orders").select("id", { count:"exact", head:true }).eq("business_id", business.id).is("archived_at", null).or("status.in.(sent,viewed,shared),picked_up_at.not.is.null")` → `{ count }` (anderer `scope` ⇒ 400 `invalid_scope`). **POST um einen Zweig erweitert:** Body `{ scope:"all-done", archive:true }` (`archive` weiter strikt `=== true`) ⇒ **EIN UPDATE ohne IDs/Cap** `.from("orders").update({ archived_at: now }).eq("business_id", business.id).is("archived_at", null).or("status.in.(sent,viewed,shared),picked_up_at.not.is.null").select("id")` → `{ archived: data.length }`. Die Eligibility-`.or(...)`-Bedingung ist in GET, all-done-POST und IDs-POST **exakt identisch** (geteilte `ARCHIVABLE_STATUSES`, kein Drift). **Der bestehende `{ ids, archive:true }`-Pfad bleibt unverändert.** **(2)** Dropdown „Archiv ▾" ([order-bulk-archive.tsx](components/order-bulk-archive.tsx), nur Hauptlisten-Scope): dritter Eintrag „Alle erledigten archivieren" (`orders.archiveAllDone`, neue `ArchiveAllDoneItem`-Komponente) → `GET ?scope=all-done` → `count N`; `N === 0` ⇒ nur Hinweis `orders.noneToArchive` (KEINE Aktion), `N > 0` ⇒ Bestätigung `orders.confirmArchiveAllDone {n}` (Ja/Abbrechen) → `POST { scope:"all-done", archive:true }` → bei `ok` `router.refresh()`. `window.confirm`/`window.alert` (Projekt-Konvention; das Menü schließt beim Klick), minimaler busy-Guard + try/catch/finally (kein Hängen). Kein `<form>`, kein `any`. **(3)** i18n `orders.archiveAllDone`/`confirmArchiveAllDone`/`noneToArchive`. `pnpm typecheck` + `pnpm build` grün.

> **Schritt 3b-2b — „Alle auswählen" pro aktivem Filter (by-filter-Bulk, über alle Seiten):** der riskanteste Bulk-Pfad — eine Massen-Archivierung, die GENAU den aktiven Listen-Filter rekonstruiert. Keine Migration. **Geteilter `draftWithMedia`-Helfer:** die `status='new'`-Menge (Entwurf MIT ≥1 Medium) wandert aus `page.tsx` inline in `getDraftWithMediaIds(supabase, { businessId, archived })` ([lib/orders/orders-query.ts](lib/orders/orders-query.ts), gibt `Set<string>`); genutzt von `page.tsx` (Badge + `new`-Filter) UND dem by-filter-Bulk (`new` braucht EXAKT dieselbe Menge, sonst trifft die Massen-Archivierung eine andere Menge als die Liste anzeigt). `page.tsx` bleibt bit-identisch (gleiche Query, gleiches Set). `buildFilteredOrdersQuery` bekommt eine optionale `head?: boolean`-Option (nur zusammen mit `count`; ohne `head` bleibt das Select-Argument `{ count }` ⇒ Liste unverändert) für den reinen Count des Bulk-GET. **(1)** [archive-bulk/route.ts](app/api/portal/orders/archive-bulk/route.ts): geteilter `resolveFilter(rawStatus, rawQuick)` (MIRROR der Listen-Logik: Quick hat Vorrang; null/undefined = Achse aus; vorhandener, aber ungültiger Wert ⇒ 400 `invalid_filter`). **`GET ?scope=filter&status=&quick=`** → zählt die archivierbaren AKTIVEN Treffer des Filters: `buildFilteredOrdersQuery({ businessId, status, quick, archived:false, draftWithMediaIds (nur bei `new`, sonst leer), selectCols:"id", count:"exact", head:true }).or("status.in.(sent,viewed,shared),picked_up_at.not.is.null")` → `{ count }` (exakt, head-only). **`POST { scope:"filter", status?, quick?, archive:true }`** (archive strikt `=== true`) → **SELECT-THEN-UPDATE** (die Embed-Filter `in_progress`/`creating`/`ready`/`failed` lassen sich NICHT als UPDATE-WHERE ausdrücken): (a) IDs auflösen via `buildFilteredOrdersQuery({…selectCols:"id"}).or(<Eligibility>).limit(MAX_FILTER_RESOLVE=5000)` (ohne `.range()`); leer ⇒ `{ archived:0 }`; (b) `update({archived_at}).in("id", ids).eq("business_id",<Session>).is("archived_at",null).or(<Eligibility>).select("id")` → `{ archived: data.length }` (Eligibility + Betrieb + „noch nicht archiviert" ERNEUT als Sicherheits-Boden gegen Race/Drift; `{ archived }` immer der TATSÄCHLICHE Count). Die Eligibility-`.or(...)` ist in ALLEN Pfaden (GET all-done/filter, POST all-done/filter/ids) byte-identisch (geteiltes `ARCHIVABLE_STATUSES`-Template). **3a-IDs- und 3b-2a-all-done-Pfade unverändert.** `business_id` NUR aus der Session (§14.2), AUTHENTICATED + RLS, kein `service_role`. **(2)** [order-bulk-archive.tsx](components/order-bulk-archive.tsx): der Bulk-Select-Context trägt zusätzlich `status`/`quick` (Prop aus `page.tsx`) + `allFilteredCount: number|null` + `selectAllFiltered(count)`; eine Einzelauswahl verlässt den All-gefiltert-Modus, „Alle auswählen" leert die Einzelauswahl, ein Listen-/Filter-Wechsel (`[idsKey, filterKey]`) resettet alles. Die fixierte `BulkArchiveBar` ist jetzt im GANZEN Select-Mode sichtbar (damit „Alle auswählen" auch ohne Einzelauswahl erreichbar ist — vorher erst ab ≥1): leer ⇒ nur „Alle auswählen"; IDs ⇒ „{n} ausgewählt" + Alle auswählen + Archivieren + aufheben; All-gefiltert ⇒ „Alle {N} ausgewählt" + Archivieren + aufheben. „Alle auswählen" → `GET ?scope=filter&status=&quick=` → N; **N===0 ⇒ nur Hinweis** `orders.noneToArchive` (kein Modus), N>0 ⇒ All-gefiltert-Modus; „Archivieren" verzweigt: IDs-Modus → `POST { ids }` (3a), All-gefiltert → Bestätigung `orders.confirmArchive {N}` → `POST { scope:"filter", status, quick }` → bei `ok` `exitSelectMode()` + `router.refresh()`. Nur Hauptlisten-Scope. Kein `<form>`, kein `any`. **(3)** i18n `orders.selectAllFiltered` („Alle auswählen") / `orders.allFilteredSelected` („Alle {n} ausgewählt"); `orders.confirmArchive` wiederverwendet. **KEIN Bulk-Unarchive.** `pnpm typecheck` + `pnpm build` grün.
...
> **UI-Fix — Auswahl-Toolbar passt auf Mobile (kein horizontaler Überlauf):** REINE CSS-Layout-Änderung ([app/globals.css](app/globals.css), Mobile-Media-Query `.bulk-bar`), keine Logik. Die fixierte Auswahl-Modus-Toolbar (Label + bis zu drei Aktionen „Alle auswählen"/„Archivieren"/„Auswahl aufheben") lief auf schmalem Viewport (~360–390px) rechts über den Rand — der dritte Button war abgeschnitten. Ursache: die `.bulk-bar-actions` (`flex-shrink:0`, nowrap-Buttons) passten unter `justify-content:space-between` nicht neben das nowrap-Label. Fix: auf Mobile die Bar **vertikal stapeln** (`flex-direction:column; align-items:stretch`, Label zentriert oben), die Aktionen mit `flex-wrap:wrap` umbrechen lassen und die Buttons `flex:1 1 auto` füllen lassen — alle Controls vollständig sichtbar/tappbar, kein Überlauf. Container war bereits viewport-breit begrenzt (`left/right:12px` + globales `border-box`). Desktop unverändert. `pnpm typecheck` + `pnpm build` grün.

> **`bakeBusinessPhotoFrame`: VORHER/NACHHER-Label fontsize 76→130, boxborderw 24→36**

> **Fix — Detail-Header-Badge bekommt `reel_status` (Konsistenz mit dem Listen-Badge bei `generated`):** NUR [app/portal/orders/[id]/page.tsx](app/portal/orders/[id]/page.tsx), kein Backend, keine Migration. Der Header-`<OrderStatusBadge>` der Detailseite bekam **kein** `reelStatus`-Prop ⇒ bei `status='generated'` defaultete das zusammengesetzte Badge (`reelBadge(null)`, Schritt C) immer auf „Wird erstellt …", unabhängig vom echten `reel_status` — während das Listen-Badge den Wert durchreicht. Die Detailseite **lädt `booklets.reel_status` ohnehin schon server-seitig** (der `canRenderReel`-Zweig deckt `generated`; AUTHENTICATED + RLS, kein `service_role`), also nur diesen bereits geladenen Wert zusätzlich ans Header-Badge gereicht: `reelStatus={reelStatus}` (die Variable ist `ReelStatus`, non-null, Default `"pending"` ⇒ zu `string | null` zuweisbar; **keine** Extra-Query nötig). `hasMedia` unverändert; `reelStatus` wirkt im Badge **nur** auf `generated` (draft nutzt `hasMedia`, sent/viewed/shared sind grün) ⇒ für alle anderen Status folgenlos. `RenderingProgress` ruft bei `ready`/`failed` bereits `router.refresh()` ⇒ der Server-gerenderte Badge zieht dann nach, keine separate Live-Reaktivität nötig. Sonst NICHTS geändert. `pnpm typecheck` + `pnpm build` grün.

> **Schritt A — „Geflaggt" vom Quick-Chip in die Status-Dropdown-Option verschoben:** NUR Frontend ([components/order-status-filter.tsx](components/order-status-filter.tsx), [app/portal/orders/page.tsx](app/portal/orders/page.tsx)), keine Migration, **kein Query-Change** — die `quick=flagged`-Logik (`buildOrdersUrl`/`buildFilteredOrdersQuery`/`QUICK_FILTERS` in [lib/orders/filters.ts](lib/orders/filters.ts)) bleibt unverändert. Die frühere Chip-Zeile (`<OrderQuickFilters>`) entfällt; der Filter „Geflaggt" („Abgeholt, Booklet nicht versendet") wird jetzt als **letzte, auswählbare Option** im Status-`<select>` geboten (Wert `__flagged__`, davor eine **deaktivierte Trenner-Option** `__sep__` „──────" zur optischen Abgrenzung). `selectValue`: `quick === 'flagged'` ⇒ `__flagged__` (ersetzt die frühere deaktivierte `__quick__`-Platzhalter-Option), sonst der Status bzw. „all". `onChange` drei Fälle: `__flagged__` → `buildOrdersUrl({ quick: 'flagged' })`; `isStatusFilter(next)` → `buildOrdersUrl({ status: next })` (droppt `quick`); sonst („all") → `buildOrdersUrl({ status: null })` (droppt beides); der Trenner ist `disabled` ⇒ feuert nie. Label „Geflaggt" über den bestehenden Key `orders.quickFlagged`. **Cleanup:** [components/order-quick-filters.tsx](components/order-quick-filters.tsx) `git rm` (nirgends mehr importiert), `.quick-filter*`-CSS aus [app/globals.css](app/globals.css) entfernt, die jetzt verwaisten i18n-Keys `orders.filterQuickActive` + `orders.quickLabel` aus [lib/i18n/de.ts](lib/i18n/de.ts) entfernt. Kein `<form>` (`<select>` = reine Navigation), kein `any`. `pnpm typecheck` + `pnpm build` grün.

> **Schritt B — Auftragslisten-Suche (`?q=`, Freitext über Name/E-Mail/Telefon/externe Referenz):** Suchleiste über der Liste, server-seitig, sanitisiert gegen `.or()`-Injection. **Keine Migration**, **kein Webhook berührt**; `business_id` weiter nur aus Session (AUTHENTICATED + RLS). **(1)** [lib/orders/orders-query.ts](lib/orders/orders-query.ts): `FilteredOrdersOptions` um optionales `q?: string`; **NACH** dem exklusiven Filter-if/else (additiv, vor `return`) ein Block, der `q` säubert und — falls etwas übrig bleibt — als `.or("customer_name.ilike.*<safe>*,external_ref.ilike.…,customer_email.ilike.…,customer_phone.ilike.…")` anhängt. ⚠️ Das `replace(/[^\p{L}\p{N}@.+\-_ ]/gu, "")` **IST** die Injection-Absicherung: es lässt NUR Unicode-Buchstaben/Ziffern + `@ . + - _` + Leerzeichen durch und strippt alle PostgREST-`.or()`-Sonderzeichen (`,` `(` `)` `*` `\` `:` `"` …); leer nach dem Strippen ⇒ kein Filter. `customer_email`/`customer_phone` müssen NICHT in `selectCols` stehen (PostgREST filtert auch auf nicht selektierte Spalten). Da der Builder `count` UND `data` speist, greift `q` automatisch auf die Pagination-Gesamtzahl. **(2)** [lib/orders/filters.ts](lib/orders/filters.ts): `buildOrdersUrl` um `q?: string | null` (gesetzt ⇒ `?q=<encoded>`, **unabhängig** von status/quick; deren Exklusivität bleibt; leer ⇒ kein `q`). **(3)** [components/order-search-bar.tsx](components/order-search-bar.tsx) (NEU, Client): `<input>` (kein `<form>`) mit EIGENEM lokalem State (init aus `initialQ`, **synct NICHT** bei jedem Re-Render aus der URL ⇒ Fokus/Cursor bleiben beim Tippen). `onChange` setzt den State SOFORT + DEBOUNCED ~300 ms `router.push(buildOrdersUrl({ q: value.trim() || null, archived }))` — **KEIN `page`-Param** (Reset auf Seite 1), **KEIN status/quick** ⇒ Suchen ist ein **Override** der aktiven Filter-Auswahl. Clear-(x) bei Text → State leeren + `router.push(buildOrdersUrl({ archived }))`. Props `initialQ`/`archived`. **(4)** [app/portal/orders/page.tsx](app/portal/orders/page.tsx): `q` aus `searchParams` (Next-15-`Promise`) → `buildFilteredOrdersQuery({ …, q })`; `<OrderSearchBar initialQ={qParam ?? ""} archived>` im Header (wo die entfernte Chip-Zeile war); aktive Suche zählt wie ein aktiver Filter (`hasActiveFilter`/`showFilter` ⇒ die Leiste bleibt bei 0 Treffern editierbar); Overshoot-Redirect + `OrdersPagination` + `OrderBulkSelectProvider` bekommen `q` durchgereicht. **(5)** [components/orders-pagination.tsx](components/orders-pagination.tsx): Prop `q` in die `buildOrdersUrl`-Seiten-Links (wie status/quick/archived). **(6) Konsistenz Bulk:** [archive-bulk/route.ts](app/api/portal/orders/archive-bulk/route.ts) — `resolveFilter` um `rawQ` erweitert (nur durchgereicht; Sanitisierung im Builder; Nicht-String ⇒ null); `GET ?scope=filter` (q aus searchParams) **und** `POST { scope:"filter" }` (q aus Body) reichen `q` an `buildFilteredOrdersQuery` durch — Count-GET UND select-then-update-POST. [order-bulk-archive.tsx](components/order-bulk-archive.tsx): Bulk-Context bekommt `q` (Prop), hängt es an den „Alle auswählen"-Count-GET (`&q=`) UND den Archive-POST (`q`) — der Einzel-IDs-Pfad bleibt unberührt; `filterKey` schließt `q` ein (Suchwechsel resettet die Auswahl). `getDraftWithMediaIds` UNVERÄNDERT (q ist orthogonal zur Medien-Existenz). **(7)** i18n `orders.searchPlaceholder`/`orders.searchClear`; CSS `.order-search-bar`/`.order-search-input`/`.order-search-clear` (volle Breite, dezent, Clear-x, mobil 16px gegen iOS-Zoom). Kein `<form>`, kein `any`. `pnpm typecheck` + `pnpm build` grün.

> **Fix — Unarchivieren-Icon (Restore-Glyph) vom Archivieren-Icon unterschieden:** NUR [components/archive-toggle.tsx](components/archive-toggle.tsx), kein Backend, keine Logik, keine Migration. In der Archivliste nutzte der „zurück in die Hauptliste"-Button (`mode="unarchive"`) optisch dasselbe Box-Symbol wie „Archivieren" in der Hauptliste — der bisherige `UnarchiveIcon` flippte nur eine kleine Chevron *innerhalb* der Box (bei 16 px kaum vom Archiv-Icon unterscheidbar). Jetzt zeigt der Restore-Glyph einen Pfeil, der oben aus der Box **heraus** nach oben zeigt (Box-Body + -Deckel nach unten gerückt, darüber Schaft `M12 3v7` + Spitze `M8.5 6.5 12 3l3.5 3.5`). Stroke-Width (1.8), Größe (16), Farbe (`currentColor`) identisch — nur der SVG-Inhalt geändert; das `ArchiveIcon` (`mode="archive"`, Hauptliste) bleibt unverändert. `pnpm typecheck` + `pnpm build` grün.
