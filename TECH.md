...
> **⚠️ TEMP — On-Device-Frame-Extraktions-Diagnose (additiv, NACH der Analyse reverten):** rein beobachtende Instrumentierung, um am iPhone sichtbar zu machen, warum bei Videos keine Vorschau-Frames entstehen. **KEINE Verhaltensänderung** an Extraktion/Upload/Schwarz-Guard — nur Diagnose-Aufrufe + ein sichtbares Panel; Kunden-Reel/andere Pfade unberührt. Jede TEMP-Zeile mit Marker `// TEMP_FRAMEDIAG` (sauberer Revert). **(1)** [lib/media/extract-frames.ts](lib/media/extract-frames.ts): `extractVideoFrames(file, diag?)` + `waitForMetadata(video, diag?)` bekommen einen optionalen `diag: (line: string) => void`; ohne `diag` exakt heutiges Verhalten. Emittiert pro Schritt (start / meta ok|TIMEOUT / pos{i} seek→seeked→draw→`black=ratio verdict` →blob / THROW / `frames=n`). `isMostlyBlack` → `measureBlackRatio` extrahiert (gibt den gemessenen Schwarz-Anteil als Zahl zurück, **Verwerf-Schwelle BLACK_RATIO unverändert** beim Aufrufer: `ratio > BLACK_RATIO`). **(2)** [app/portal/orders/[id]/capture.tsx](app/portal/orders/[id]/capture.tsx): `framediagAppend` schreibt Zeilen mit Zeitstempel nach `sessionStorage["__framediag"]` (SSR-safe, überlebt `router.refresh()` — die Extraktion startet erst NACH dem refresh); `extractAndUploadFrames` reicht `diag` an `extractVideoFrames` durch + loggt `videoPath` / `extract -> frames=n` / `upload pos{i} -> ok|FAIL`. Sichtbares `<FrameDiagPanel>` (fixed unten, monospace, **selektierbar** für iPhone-Copy, „Kopieren"/„Leeren", liest sessionStorage alle 750 ms). Kein `any` (`diag: (line:string)=>void`, Panel-State `string[]`), kein `<form>`. `pnpm typecheck` + `pnpm build` grün.
